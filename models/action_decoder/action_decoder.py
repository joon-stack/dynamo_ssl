...
class Action_Decoder(nn.Module):
    def __init__(
        ...
        #     action_dim = 9
        # elif args.dataset == 'blockpush':
        #     pass
        # elif args.dataset == 'pusht':
        #     pass
        # elif args.dataset == 'libero':
        #     pass
        # elif args.dataset == 'langtable':
        #     action_dim = 2
        

        encoder_layer = nn.TransformerEncoderLayer(d_model=model_dim, nhead=num_heads, dim_feedforward=ffn_dim, batch_first=True, dropout=0.1)
        self.action_decoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        self.slot_proj = nn.Linear(slot_dim, model_dim)
        self.fc = nn.Linear(model_dim, act_dim)
        self.action_token = nn.Parameter(nn.init.normal_(torch.empty(1, model_dim)))

        self.ckp_path = ckp_path
        
    def forward(self, slots):
        # slots: [B, num_slots, slot_dim = 256]
        # gt_actions: [B, action_dim]
        B = slots.shape[0]
        action_token = self.action_token.unsqueeze(0).repeat(B, 1, 1) # [1, model_dim] -> [B, 1, model_dim]
        slots = self.slot_proj(slots) # [B, num_slots, slot_dim] -> [B, num_slots, model_dim]
        model_input = torch.cat([slots, action_token], axis=1) # [B, num_slots + 1, model_dim], last token is action
        output = self.action_decoder(model_input)[:, -1] # [B, model_dim]
        pred_actions = self.fc(output) # [B, model_dim] -> [B, action_dim]
...
class Visual_Encoder(nn.Module):
    ...
    def load_model(self, args):
        ...
        elif args.encoder == "dinov2-vits-14":
            model = torch.hub.load("facebookresearch/dinov2", "dinov2_vits14")
        elif args.encoder == "dinov2-vitb-14-reg":
            model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitb14_reg")
        elif args.encoder == "dinov2-vits-14-reg":
        ...
        elif args.encoder == "dinov2-vitl-14":
            model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitl14")
        elif args.encoder == "dinov2-vitg-14":
...
class DINOSAURpp(nn.Module):
    def __init__(self, args):
        ...
        self.ISA = args.ISA
        if self.ISA:
            self.slot_encoder = ISA(args, input_dim=self.feature_dim)
        else:
            self.slot_encoder = SA(args, input_dim=self.feature_dim)
