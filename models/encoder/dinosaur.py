...
class Loss_Function(nn.Module):
    def __init__(self, token_num, num_slots):
        super().__init__()
    ...
    def forward(self, reconstruction, masks, target):
        # :args reconstruction: (B, token, feature_dim)
        # :args masks: (B, S, token)
        # :args target: (B, token, feature_dim)
...
class Visual_Encoder(nn.Module):
    def __init__(self, resize_to, encoder, patch_size, token_num, feature_dim):
        super().__init__()
    ...
    def load_model(self):
        assert self.resize_to[0] % self.patch_size == 0
        assert self.resize_to[1] % self.patch_size == 0
        
        if self.encoder == "dino-vitb-8":
            model = torch.hub.load("facebookresearch/dino:main", "dino_vitb8")
        elif self.encoder == "dino-vitb-16":
            model = torch.hub.load("facebookresearch/dino:main", "dino_vitb16")
        elif self.encoder == "dinov2-vitb-14":
            model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitb14")
        elif self.encoder == "dinov2-vits-14":
            model = torch.hub.load("facebookresearch/dinov2", "dinov2_vits14")
        elif self.encoder == "dinov2-vitb-14-reg":
            model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitb14_reg")
        elif self.encoder == "dinov2-vits-14-reg":
            model = torch.hub.load("facebookresearch/dinov2", "dinov2_vits14_reg")
        elif self.encoder == "sup-vitb-16":
            model = timm.create_model("vit_base_patch16_224", pretrained=True, img_size=(self.resize_to[0], self.resize_to[1]))
        elif self.encoder == "dinov2-vitl-14":
            model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitl14")
        elif self.encoder == "dinov2-vitg-14":
...
class Decoder(nn.Module):
    def __init__(self, slot_dim, feature_dim):
        super().__init__()
        ...
        self.layer4 = nn.Linear(hidden_dim, feature_dim + 1)
    ...
    def forward(self, slot_maps):
        ...
        slot_maps = self.layer4(slot_maps)               # (B * S, token, feature_dim + 1)

        return slot_maps
    
class ISA(nn.Module):
    def __init__(self, num_slots, slot_att_iter, slot_dim, query_opt, resize_to, patch_size, input_dim):
        super().__init__()
        ...
        self.res_h = resize_to[0] // patch_size
        self.res_w = resize_to[1] // patch_size
...
class SA(nn.Module):
    def __init__(self, slot_dim, slot_att_iter, num_slots, input_dim, query_opt):
...
class DINOSAURpp(nn.Module):
    def __init__(
        ...
        # === SA related ===
        self.query_opt = query_opt
        self.slot_att_iter = slot_att_iter

        # === Visual encoder related ===
        self.resize_to = resize_to
        self.encoder = encoder
        self.patch_size = int(encoder.split("-")[-1])
        ...
        self.ISA = ISA
        if self.ISA:
            self.slot_encoder = ISA(
                                    input_dim=self.feature_dim,
                                    num_slots=self.num_slots, 
                                    slot_dim=self.slot_dim, 
                                    slot_att_iter=self.slot_att_iter, 
                                    query_opt=self.query_opt, 
                                    resize_to=self.resize_to, 
                                    patch_size=self.patch_size, 
                                    )
        else:
            self.slot_encoder = SA(
                                    input_dim=self.feature_dim, 
                                    slot_dim=self.slot_dim, 
                                    slot_att_iter=self.slot_att_iter, 
                                    num_slots=self.num_slots,
    ...
    def reconstruct_feature_map(self, slot_maps):
        # :arg slot_maps: (B, S, token, feature_dim + 1)
        #
        # :return reconstruction: (B, token, feature_dim)
        # :return masks: (B, S, token)

        B = slot_maps.shape[0]

        channels, masks = torch.split(slot_maps, [self.feature_dim, 1], dim=-1)  # (B, S, token, feature_dim), (B, S, token, 1)
        masks = masks.softmax(dim=1)                                # (B, S, token, 1)

        reconstruction = torch.sum(channels * masks, dim=1)         # (B, token, feature_dim)
    ...
    def forward(self, features):
        # :arg features: (B, token, feature_dim)
        #
        # :return reconstruction: (B, token, feature_dim)
        # :return slots: (B, S, D_slot)
        # :return masks: (B, S, token)
        ...
        if self.ISA: 
            ...
            # slot_maps = self.slot_decoder(slot_maps)                            # (B, S, token, feature_dim + 1)
        ...
        # reconstruction, masks = self.reconstruct_feature_map(slot_maps)     # (B, token, feature_dim), (B, S, token)
